...
function! helpeek#window#new() abort
    ...
    function! window.open(bufnr) abort
        ...
        call self.logger.log('open: ' . self._window)

        let self._border = helpeek#window#add_border(self._width, self._height, self._row, self._col, event_service)
    endfunction

    function! window.close() abort
        if !empty(self._window) && nvim_win_is_valid(self._window)
            call nvim_win_close(self._window, v:true)
            call self.logger.log('close : ' . self._window)
        endif
...
function! helpeek#window#add_border(width, height, row, col, event_service) abort
    let window = {
